...
rule cutadapt:
  ...
  shell:
    """
    cutadapt --cores {threads} -a {adapter} --overlap 8 -o {output} {input}
    """
...
rule dragmap:
  ...
  shell:
    """
    dragen-os --RGSM {wildcards.sample} --num-threads {threads} -r {params.refdict} -1 {input} --output-file-flowcell {output} 2> {log} | 
    samtools view -bS | 
...
rule markduplicates:
   ...
   shell:
     """
     gatk MarkDuplicatesSpark --conf "spark.executor.cores={threads}" --tmp-dir {temp} --input {input} --output {output.bam} -M {params.metrics_out}
     """

rule mosdepth:
    input: "mapping/{sample}/{flowcell}_markdup.bam"
    output:
        "mapping/{sample}/mosdepth/{flowcell}.mosdepth.global.dist.txt",
        "mapping/{sample}/mosdepth/{flowcell}.mosdepth.region.dist.txt",
        "mapping/{sample}/mosdepth/{flowcell}.mosdepth.summary.txt",
        "mapping/{sample}/mosdepth/{flowcell}.regions.bed.gz"
    log: "logs/{sample}/mosdepth/{flowcell}.log"
    benchmark: "benchmarks/{sample}/mosdepth/{flowcell}.tsv"
    params:
        by = "500",
        flowcell = "mapping/{sample}/mosdepth/{flowcell}",
