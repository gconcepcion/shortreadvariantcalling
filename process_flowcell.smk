...
if 'cutadapt' in config['flowcell_targets']:
    targets.extend([f"cutadapt/{sample}/{flowcell}_trimmed.{suffix}" for suffix in ['fastq.gz']])  
...
if 'markdups' in config['flowcell_targets']:
    targets.extend([f"mapping/{sample}/{flowcell}_markdup.{suffix}" for suffix in ['bam', 'bam.bai']])

if 'mosdepth' in config['flowcell_targets']:
    targets.extend([f"mapping/{sample}/mosdepth/{flowcell}.{suffix}" for suffix in ['mosdepth.global.dist.txt', 
